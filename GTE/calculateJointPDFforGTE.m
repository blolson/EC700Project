function P = calculateJointPDFforGTE(D, varargin)
...
dims = [size(D,2), size(D,2), bins*ones(1, dims)];
...
for i = 1:size(D,2)
    ...
    for j = (i+1):size(D,2)
        ...
        coordsIJ = [i*ones(size(multDi,1),1), j*ones(size(multDi,1),1), multDj(:,1:end), multDi(:,(2-IFT):(end-IFT))];
        indxIJ = (coordsIJ-1)*multipliers+1;
        P(:) = P(:) + histc(indxIJ, 1:numel(P));
        coordsJI = [j*ones(size(multDj,1),1), i*ones(size(multDi,1),1), multDi(:,1:end), multDj(:,(2-IFT):(end-IFT))];
